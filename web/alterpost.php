...
if (!$message_array = get_message_with_time($message_id)) {
    ...
    if (!can_user_edit_post($user_array['user_id'], $user_array['user_sysop'], $section_array['user_id'], $message_array['user_id'], $message_array['message_time'])) {
    ...
    if (!$moderator_name = get_username($section_array['user_id'])) {
            show_error($_SERVER['PHP_SELF']."get_user_array failed for moderator <br> SESSION is ".print_r($_SESSION, true));
    }

    if (!$breadcrumbs = get_breadcrumbs_topic($section_array['section_id'])) {
        show_error($_SERVER['PHP_SELF']."get_breadcrumbs_topic failed <br>".print_r($section_array, true));
    }
