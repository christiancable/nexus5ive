...
if ($waiting_users_array) {
    ...
    foreach ($waiting_users_array as $waiting_user_id) {
        ...
        if ($accept) {
            mail(SYSOP_MAIL, "New Account Created", "Added User:\n".print_r($current_user_array, true), "From: ".SYSOP_MAIL);
        } else {
            mail(SYSOP_MAIL, "New Account Rejected", "Rejected User:\n".print_r($current_user_array, true), "From: ".SYSOP_MAIL);
        }
    }
...
header("Location: http://".$_SERVER['HTTP_HOST'].get_bbsroot()."validate_accounts.php");
