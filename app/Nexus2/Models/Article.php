...
class Article
{
    const DATE_LINE_START       = "\e\x01";
    const USER_LINE_START       = "\e\x02";
    const SUBJECT_LINE_START    = "\e\x03";
    ...
    public function parse(string $string): void
    {
        ...
        $rawPosts = explode(self::DATE_LINE_START, $this->_content);

        foreach ($rawPosts as $rawPost) {
            if ($rawPost != '') {
                if ($this->isComment($rawPost)) {
                    $this->_comments[] = $this->parsePost($rawPost);
                } else {
                    $this->_preamble .= $rawPost;
                }
            }
        }
    }
